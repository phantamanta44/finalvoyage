...
public class FVHellProvider extends WorldProviderHell {

    public static Supplier<WorldProvider> factory;

    private final WorldProvider backing;

    public FVHellProvider() {
        this.backing = factory.get();
        FVMod.LOGGER.info("Hell provider wrapping " + backing.getClass().getName());
    }

    @Override
    public void createBiomeProvider() {
        backing.registerWorld(worldObj);
        ((WorldProviderHell)backing).createBiomeProvider();
    }
    ...
    public boolean canCoordinateBeSpawn(int x, int z) {
        return this.getSpawnPointFor(x, z) != null;
    }
    ...
    public BlockPos getRandomizedSpawnPoint() {
        ...
        for(int i = 0; i < 10 && spawnPoint == null; ++i) {
            BlockPos pos = backing.getRandomizedSpawnPoint();
            spawnPoint = this.getSpawnPointFor(pos.getX(), pos.getZ());
        }

        return spawnPoint != null?spawnPoint:backing.getRandomizedSpawnPoint();
    }
    ...
    private BlockPos getSpawnPointFor(int x, int z) {
        BlockPos pos = new BlockPos(x, 127, z);

        for(boolean seenAir = false; pos.getY() > 0; pos = pos.down()) {
            IBlockState state = this.worldObj.getBlockState(pos);
            if(seenAir) {
                if(!state.getBlock().isAir(state, this.worldObj, pos)) {
                    return state.getBlock() != Blocks.LAVA?pos:null;
                }
